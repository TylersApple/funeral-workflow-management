/*
  # Complete Funeral Workflow Management System Database Recreation

  1. Cleanup
    - Drop all existing tables and types
    - Drop all sequences and functions
...
DROP TABLE IF EXISTS fw_email_logs CASCADE;
DROP TABLE IF EXISTS fw_status_history CASCADE;
DROP TABLE IF EXISTS fw_documents CASCADE;
DROP TABLE IF EXISTS fw_records CASCADE;
DROP TABLE IF EXISTS fw_users CASCADE;
DROP TABLE IF EXISTS fw_system_settings CASCADE;

-- Drop existing types
DROP TYPE IF EXISTS fw_record_status CASCADE;
DROP TYPE IF EXISTS fw_user_role CASCADE;

-- Drop existing sequences
DROP SEQUENCE IF EXISTS fw_record_seq CASCADE;

-- Drop existing functions
DROP FUNCTION IF EXISTS update_fw_updated_at() CASCADE;
DROP FUNCTION IF EXISTS get_status_percentage(fw_record_status) CASCADE;
DROP FUNCTION IF EXISTS handle_status_change() CASCADE;

-- Create sequence first (before it's referenced)
CREATE SEQUENCE fw_record_seq START 1;
...
CREATE TABLE fw_status_history (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  record_id uuid REFERENCES fw_records(id) ON DELETE CASCADE,
...
CREATE TABLE fw_system_settings (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- RLS Policies for fw_users (simplified to avoid recursion)
...
-- Allow all operations temporarily for setup (can be restricted later)
CREATE POLICY "Allow all operations on fw_users"
  ON fw_users
  FOR ALL
  TO public
  USING (true);

CREATE POLICY "Allow all operations on fw_records"
  ON fw_records
  FOR ALL
  TO public
  USING (true);

CREATE POLICY "Allow all operations on fw_documents"
  ON fw_documents
  FOR ALL
  TO public
  USING (true);

CREATE POLICY "Allow all operations on fw_status_history"
  ON fw_status_history
  FOR ALL
  TO public
  USING (true);

CREATE POLICY "Allow all operations on fw_email_logs"
  ON fw_email_logs
  FOR ALL
  TO public
  USING (true);

CREATE POLICY "Allow all operations on fw_system_settings"
  ON fw_system_settings
  FOR ALL
  TO public
  USING (true);
...
CREATE INDEX idx_fw_records_status ON fw_records(status);
CREATE INDEX idx_fw_records_created_at ON fw_records(created_at);
CREATE INDEX idx_fw_records_assigned_to ON fw_records(assigned_to);
CREATE INDEX idx_fw_records_next_email_due ON fw_records(next_email_due);
CREATE INDEX idx_fw_documents_record_id ON fw_documents(record_id);
CREATE INDEX idx_fw_status_history_record_id ON fw_status_history(record_id);
CREATE INDEX idx_fw_email_logs_record_id ON fw_email_logs(record_id);
...
-- Function to handle status changes and update percentages
CREATE OR REPLACE FUNCTION handle_status_change()
RETURNS TRIGGER AS $$
BEGIN
  -- Update progress percentage based on status
  CASE NEW.status
    WHEN 'record_created' THEN NEW.progress_percentage = 1;
    WHEN 'funeral_arrangement' THEN NEW.progress_percentage = 10;
    WHEN 'documents_sent_home_affairs' THEN NEW.progress_percentage = 15;
    WHEN 'quotation_accepted' THEN NEW.progress_percentage = 20;
    WHEN 'payment_made' THEN NEW.progress_percentage = 30;
    WHEN 'payment_arrangement' THEN NEW.progress_percentage = 40;
    WHEN 'payment_reminder_1' THEN NEW.progress_percentage = 50;
    WHEN 'payment_reminder_2' THEN NEW.progress_percentage = 60;
    WHEN 'payment_reminder_final' THEN NEW.progress_percentage = 70;
    WHEN 'agreement_breached' THEN NEW.progress_percentage = 70;
    WHEN 'funeral_completed' THEN NEW.progress_percentage = 100;
    ELSE NEW.progress_percentage = 1;
  END CASE;
...
CREATE TRIGGER update_fw_records_updated_at
  BEFORE UPDATE ON fw_records
  FOR EACH ROW
  EXECUTE FUNCTION update_fw_updated_at();

CREATE TRIGGER handle_fw_record_status_change
  BEFORE UPDATE ON fw_records
  FOR EACH ROW
